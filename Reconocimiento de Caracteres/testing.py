...
from final_net import stitch_tiles_to_image

# UNet matching the training script `final_net.py`
...
def main():
    ...
    parser.add_argument('--model', type=str, default=str(Path(__file__).parent / 'visualizaciones' / 'checkpoint_epoch_60.pth'))
    parser.add_argument('--testing-dir', type=str, default=str(Path(__file__).parent / 'testing'))
    parser.add_argument('--out-dir', type=str, default=str(Path(__file__).parent / 'visualizaciones' / 'testing_preds'))
    parser.add_argument('--target-size', type=int, nargs=2, default=[256,256])
    parser.add_argument('--stride', type=int, default=0, help='Sliding window stride in pixels; default 0 -> 50% overlap')
